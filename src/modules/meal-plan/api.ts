...
export function generatePlan(data: GenerateRequest) {
  return http.post<WeeklyMealPlan>('/meal-plans/generate', data)
}

/** 获取当前周计划（支持传 weekStartDate 查询指定周） */
export function getCurrentPlan(weekStartDate?: string) {
  return http.get<WeeklyMealPlan>('/meal-plans/current', weekStartDate ? { weekStartDate } : undefined)
}

/** 获取计划详情 */
export function getPlanDetail(planId: number) {
  return http.get<WeeklyMealPlan>(`/meal-plans/${planId}`)
}

/** 替换计划中某餐项 */
export function replaceItem(planId: number, itemId: number, recipeId: number) {
  return http.put(`/meal-plans/${planId}/items/${itemId}/replace`, { recipeId })
}

/** 添加计划项（从菜品库选择） */
export function addItem(planId: number, data: { recipeId: number, mealDate: string, mealType: string, crowdType?: string }) {
  return http.post(`/meal-plans/${planId}/items`, data)
}

/** 删除计划项 */
export function deleteItem(planId: number, itemId: number) {
  return http.delete(`/meal-plans/${planId}/items/${itemId}`)
}

/** 手动添加自定义菜品项 */
export function manualAddItem(planId: number, data: { recipeName: string, mealDate: string, mealType: string, crowdType?: string }) {
  return http.post(`/meal-plans/${planId}/items/manual`, data)
}

/** 确认计划 */
export function confirmPlan(planId: number) {
  return http.post<ConfirmPlanResult>(`/meal-plans/${planId}/confirm`)
}

/** 获取备菜计划 */
export function getPrepPlan(planId: number) {
  return http.get<PrepPlan>(`/meal-plans/${planId}/prep-plan`)
}

/** 更新备菜项状态 */
export function updatePrepItemStatus(planId: number, itemId: number, status: string) {
  return http.put(`/meal-plans/${planId}/prep-plan/items/${itemId}/status`, { status })
}

/** 获取采购清单 */
export function getShoppingList(planId: number) {
  return http.get<ShoppingItem[]>(`/meal-plans/${planId}/shopping-list`)
}

/** 更新采购项状态 */
export function updateShoppingItem(planId: number, itemId: number, purchased: boolean) {
  return http.put(`/meal-plans/${planId}/shopping-list/items/${itemId}`, { purchased })
}
