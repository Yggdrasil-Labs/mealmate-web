...
| MealPlan | 周餐计划生成、调整、确认与周导航 | `src/modules/meal-plan/` | WeeklyMealPlan, MealPlanItem, DayMeal |
| Prep | 备菜计划与采购清单（由确认后的周计划派生） | `src/modules/prep/` | PrepPlan, PrepItem, ShoppingItem |
...
flowchart LR
  ...
  Shell --> MealPlan["MealPlan 周计划"]
  Shell --> Prep["Prep 备菜/采购"]
  Family -.->|"偏好影响推荐"| MealPlan
  Recipe -->|"菜品选择"| MealPlan
  MealPlan -->|"确认后派生"| Prep
```
...
- `Prep` 依赖 `MealPlan` store 获取当前 planId，不直接调用 meal-plan API
